...
export function SavedViews() {
  ...
  return (
    <div className="space-y-1.5 rounded-[var(--pc-radius)] border border-[var(--pc-border)] bg-[var(--pc-surface)] px-3 py-2">
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
        <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--pc-text-muted)]">
          Views
        </span>
        <span className="pc-helper-text">
          Use presets for common triage queues, or save the current filters for
          a repeated review.
        </span>
      </div>
      <div className="flex items-center gap-1.5 overflow-x-auto">
        ...
        {(userViews.data ?? []).length > 0 && (
          <span
            className="mx-1 h-3 w-px bg-[var(--pc-border)]"
            aria-hidden="true"
          />
        )}
