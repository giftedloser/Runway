...
run the operator registers PilotCheck as an application in their own Entra
...
- **Microsoft Graph credentials** in the per-user app data `.env`. The
  ...
  BitLocker at rest — PilotCheck itself does not encrypt `.env`.
...
- Issues that require an attacker to already have local OS access on the
  ...
  addressed by the operator's endpoint protection, not by PilotCheck.
